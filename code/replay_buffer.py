...
class PriorReplayBuffer:
    ...
    *Note that we are indexing starting from 1*.
    ...
    def __init__(self, capacity, alpha=0.5):
        ...
        # Arrays for buffer
        self.data = {
            'obs': np.zeros(shape=(capacity, 4, 96, 96), dtype=np.float32),
            'action': np.zeros(shape=capacity, dtype=np.int32),
            'reward': np.zeros(shape=capacity, dtype=np.float32),
            'next_obs': np.zeros(shape=(capacity, 4, 96, 96), dtype=np.float32),
    ...
    def sample(self, batch_size, beta):
        ...
        # Get samples data
        norm_ans=(self.data['obs'][samples['indexes']],
                  self.data['action'][samples['indexes']],
                  self.data['reward'][samples['indexes']],
                  self.data['next_obs'][samples['indexes']],
                  self.data['done'][samples['indexes']]
                 )
        for k, v in self.data.items():
            samples[k] = v[samples['indexes']]

        return norm_ans, samples['indexes'], samples['weights']
